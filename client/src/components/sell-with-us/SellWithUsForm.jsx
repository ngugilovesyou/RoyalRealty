...
export default function SellWithUsForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await submitSellLand(formData, images);
      
      setSubmitStatus('success');
      toast.success("Your land details have been submitted successfully!");
  ...
  return (
    <>
     <Head 
        title="Sell With Us | Royal Realty"
        description="List your land for sale with Royal Realty. Fill out our simple form to connect with buyers and get expert assistance for selling your property in Kenya."
        keywords="sell land Kenya, land listing, property seller, sell property Nairobi, real estate agent Kenya, land valuation"
        type="website"
        url = "https://royalrealty.co.ke/sell"
      />
      ...
      <section className="bg-[#F6F4ED] py-24 px-6 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          ...
          >
            ...
            <h1 className="font-serif text-3xl md:text-4xl lg:text-5xl font-light text-[#222] mb-4">
              Sell Your Land With Us
            </h1>
            ...
            <AnimatePresence>
              {submitStatus && (
                <motion.div 
                  ...
                  className={`px-6 py-4 ${submitStatus === 'success' ? 'bg-green-50' : 'bg-red-50'}`}
                >
            ...
            <form onSubmit={handleSubmit} className="p-8 md:p-10 space-y-6">
              ...
              <div>
                <label htmlFor="title" className={labelClasses}>
                  Land Title / Name <span className="text-red-500">*</span>
                </label>
                <input 
                  type="text" 
                  id="title" 
                  ...
                  placeholder="Prime Residential Plot" 
                />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input 
                    ...
                    required 
                    className={inputClasses} 
                    placeholder="e.g., 1/4 acre, 50x100 ft" 
                  />
                </div>
                <div>
                  <label htmlFor="askingPrice" className={labelClasses}>
                    Asking Price (KES) <span className="text-red-500">*</span>
                  </label>
                  <input 
                    type="text" 
                    id="askingPrice" 
              ...
              <div>
                <label className={labelClasses}>
                  Land Images <span className="text-gray-400 text-xs">(Optional - Max 5 images, 5MB each)</span>
                </label>
                    ...
                    <AnimatePresence>
                      {imagePreviews.map((preview, index) => (
                        <motion.div 
                          ...
                          className="relative group aspect-square rounded-lg overflow-hidden border border-[#e0e0e0]"
                        >
                          <img 
                            src={preview} 
                            alt={`Preview ${index + 1}`} 
                          ...
                          <div className="absolute bottom-2 left-2 bg-black/50 text-white text-xs px-2 py-1 rounded">
                            {formatImageName(images[index]?.name || '')}
                          </div>
                        </motion.div>
                      ))}
                    </AnimatePresence>
                ...
                <motion.div 
                  whileHover={{ scale: imagePreviews.length >= 5 ? 1 : 1.02 }} 
                  whileTap={{ scale: imagePreviews.length >= 5 ? 1 : 0.98 }} 
                  className="relative"
                >
                  <input 
                    type="file" 
                  ...
                  <div className={`w-full p-6 border-2 border-dashed rounded-lg text-center transition ${
                    ...
                    <Upload className={`mx-auto mb-2 ${
                      imagePreviews.length >= 5 ? 'text-gray-400' : 'text-[#2F6B3C]'
                    }`} size={24} />
                    <p className={`text-sm ${
                      imagePreviews.length >= 5 ? 'text-gray-400' : 'text-[#2F6B3C]'
                    }`}>
              ...
              <div>
                ...
                <textarea 
                  ...
                  rows="4" 
                  className={inputClasses} 
                  placeholder="Tell us more about your land - any unique features, documentation available, etc." 
                />
              </div>
              ...
              <motion.button 
                ...
                whileTap={{ scale: 0.98 }} 
                className={`w-full px-8 py-4 text-sm font-medium tracking-widest uppercase rounded-md bg-[#2F6B3C] text-white hover:bg-[#3D8A4F] transition flex items-center justify-center gap-2 ${
