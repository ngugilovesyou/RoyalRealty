...
export default function ListingsPage() {
  ...
  const handleEdit = async (listing) => {
    try {
      const data = await fetchListingDetails(listing.id);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await deleteListingsBulk(selectedListings);
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-lg font-semibold text-[#2F6B3C]">
                      {listing.asking_price || listing.price ? (() => {
                          const cleaned = String(listing.asking_price || listing.price).replace(/[^0-9.]/g, '');
                          const num = parseFloat(cleaned);
                          return isNaN(num) ? listing.asking_price || listing.price : 
