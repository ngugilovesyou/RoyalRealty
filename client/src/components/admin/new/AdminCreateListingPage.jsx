...
export default function AdminCreateListingPage() {
  ...
  const buildFormDataPayload = () => {
    ...
    payload.append('title', formData.title || `${formData.county} Land`);
    payload.append('county', formData.county);
    payload.append('town', formData.town);
    payload.append('size', `${formData.size} ${formData.sizeUnit}`);
    payload.append('askingPrice', formData.askingPrice);
    ...
    images.forEach((image, index) => {
      payload.append(`image_${index}`, image);
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // With cookie-based auth there's no token to pre-check client-side —
      // an expired/missing session only surfaces as a 401 from the server.
      if (err.status === 401) {
        navigate('/admin/auth');
        return;
      }
