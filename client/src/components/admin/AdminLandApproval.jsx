...
export default function AdminLandApproval() {
  ...
  const loadApprovedLands = async () => {
    setLoading(true);
    try {
      const data = await fetchApprovedLands();
      setLands(data);
    } catch (err) {
  ...
  const handleSave = async () => {
    setActionLoading(true);
    try {
      await updateListing(selectedLand.id, {
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="text-lg font-semibold text-[#2F6B3C] mb-3">
                    {land.asking_price ? (() => {
                            const cleaned = String(land.asking_price).replace(/[^0-9.]/g, '');
                            const num = parseFloat(cleaned);
                            return isNaN(num) ? land.asking_price : 
