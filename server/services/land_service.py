...
def generate_land_slug(title, town, county):
    base = slugify(
        f"{title} {town} {county}"
    )
...
def get_approved_lands():
    ...
    try:
        ...
        for land in approved_lands:
            ...
            approved_by_admin = db.session.get(Admin, land.approved_by) if land.approved_by else None
...
def sell_land_service():
    try:
        ...
        title = request.form.get("title") or f"{county} Land"  
        slug = generate_land_slug(
    title or f"{size} Land For Sale",
    town,
    county
)
        ...
        for key in request.files:
            ...
            #     _external=True
            # ) 
                image_url = url_for("serve_upload", filename=unique_name, _external=True)
...
def approve_land_service(land_id, admin_id):
    ...
    land = db.session.get(LandSubmission, land_id)
...
def reject_land_service(land_id):
    ...
    land.approved_by = admin.id if admin else None
    land.reviewed_at = datetime.utcnow()
...
def get_land_by_slug(slug):
    land = LandSubmission.query.filter_by(
        slug=slug,
        status="approved"
    ).first()
